...
```
3. На время удалим поле ввода и кнопки, позже мы добавим их в верхнюю панель. Вот как выглядит файл `activity_main.xml` на этом этапе.
```xml
<androidx.coordinatorlayout.widget.CoordinatorLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
    xmlns:app="http://schemas.android.com/apk/res-auto"
...
Приложение будет выглядеть так:
 ...
 4. Теперь сделаем главную часть нашей анимации - поле ответа будет двигаться вместе с верхней панелью. Для этого обернем наш CardView в элемент `androidx.core.widget.NestedScrollView` с параметром `app:layout_behavior="@string/appbar_scrolling_view_behavior"`

Наличие такого элемента вместе с `CollapsingToolbarLayout` внутри `CoordinatorLayout` автоматически добавит нужную нам анимацию.
