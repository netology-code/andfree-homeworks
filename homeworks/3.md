...
1. Получим доступ к полю вывода ответа внутри метода `askWolfram`.

```kotlin
val answerText = findViewById<TextView>(R.id.answer_output)
```

2. Установим начальный текст ответа.
...
5. После его выполнения у нас есть результат, queryResult, которые нужно отобразить в интерфейсе.
...
6. Везде, где мы выводим текст в Log, выведем тот же текст в поле ответа с помощью:

```kotlin
answerText.text = "..."
```

7. В итоге наш метод выглядит так:
