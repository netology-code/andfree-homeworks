...
Дальше проверте, удачно ли выполнился запрос и есть ли какая-то информация в ответе.

```Kotlin
resultCode == RESULT_OK && data != null
```

Если все эти условия выполняются, то результат получается из ответного Intent’a:
...
1. Добавьте кнопку для воспроизведения ответа. Откройте файл `activity_main.xml`. И добавьте следующий код: 
...
2. Переведите ответ из текста в голос. Откройте файл `MainActivity.kt`. И добавьте следующий код:

Итоговый код вы можеет скопировать здесь:
...
1. Реализуйте автоматический запуск поиска с результатом распознавани. Для этого запустите функцию `askWolfram` внутри `onActivityResult` автоматически, сразу после записи вопроса в текстовое поле.
